...
# -*- coding: utf-8 -*-
"""
World Simulation v5.4 — Multi-Civilization Parallel Simulation

Runs multiple civilizations (Japan, China, Europe, Islamic) in parallel,
with inter-civilization interaction across 4 channels (trade, war,
knowledge_diffusion, disease_exchange).

This completes Requirement 11 of the original specification:
"複多項並列マルチエージェント世界シミュレーション"
(complex-polynomial parallel multi-agent world simulation).

Each civilization has its own:
- agent population (n_per_civ)
- cultural module (occupations, eras, strategy distribution)
- per-civ trajectory and chronicle

Civilizations interact via the inter_civ_interaction module.

Usage:
    python world_sim_v5_4_multi_civ.py --civs Japan,China,Europe,Islamic --n 10000
    python world_sim_v5_4_multi_civ.py --civs Japan,Europe --frequency frequent --n 15000
"""
...
def run_multi_civ_simulation(civ_names, n_per_civ=10000, n_generations=50,
    ...
    # Sample shared global events (ahistorical, frequency-mode dependent)
    events_global = []
    if enable_ahistorical:
        events_global = sample_ahistorical_events(rng, 0, n_generations * 40,
                                                    frequency_multiplier=freq_mult)

    if verbose:
        print(f"\n{'='*70}")
        print(f"Multi-Civilization Simulation: {', '.join(civ_names)}")
...
def main():
    ...
    ap.add_argument("--civs", default="Japan,China,Europe,Islamic",
    ...
    ap.add_argument("--n_generations", type=int, default=50,
                    help="Number of generations (50=2000y, 75=3000y)")
    ...
    ap.add_argument("--outdir", default="outputs_v54")
    args = ap.parse_args()
