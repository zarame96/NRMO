...
# v5.1 additions: ahistorical events + encounter mechanism
...
# Frequency multipliers for event sampling
FREQUENCY_MULTIPLIERS = {
    "sporadic":  1.0,    # historical baseline (default)
    "frequent":  3.0,    # crisis-heavy world
    "sparse":    0.3,    # peaceful world
}
...
def run_world(world_cfg, n=100000, seed=20260506, strategy_dist=None,
    ...
    Args:
        frequency_mode: "sporadic" (default) | "frequent" | "sparse"
        enable_ahistorical: include ahistorical event catalog (default True)
        encounter_year_window: (start, end) tuple for forcing encounter, or None
    """
    ...
    # Sample historical events (with frequency multiplier on a copy)
    events = sample_random_events(rng, 0, 2020, world_cfg)
    # Apply frequency multiplier by re-sampling with adjusted prob
    if freq_mult != 1.0:
        events = []
        for ev_def in RANDOM_EVENTS:
            ...
            if rng.random() < p_occur:
                ...
                events.append({"label": label, "year": yr, "magnitude": mag,
    ...
    # Sample ahistorical events
    ahistorical_events_list = []
    if enable_ahistorical:
        ahistorical_events_list = sample_ahistorical_events(
            rng, 0, 2020, frequency_multiplier=freq_mult, world_cfg=world_cfg)

    # Merge all events for unified processing
    all_events = sorted(events + ahistorical_events_list, key=lambda e: e["year"])

    # Encounter state (for Unknown-Encounter world OR forced via parameter)
    ...
    if verbose:
        ...
        print(f"  Frequency mode: {frequency_mode} (×{freq_mult})")
        print(f"  Initial religion_strength: {religion_strength:.2f}")
        print(f"  Historical events: {len(events)}")
        print(f"  Ahistorical events: {len(ahistorical_events_list)}")
        if encounter_active:
            print(f"  Encounter scheduled: year {encounter_year}")
        if ahistorical_events_list:
            print(f"  Ahistorical event sample:")
            for ev in ahistorical_events_list[:5]:
                print(f"    Year {ev['year']}: {ev['label']} ({ev['dim']}, mag={ev['magnitude']:.2f})")
    ...
    event_log = []
    encounter_log = []
    pending_secondary_effects = []  # per-agent effects from this gen
    ...
    for gen in range(1, 51):
        ...
        # Reset per-gen secondary effect accumulator
        secondary_effects_this_gen = {
        ...
        # Process all events firing in this generation window
        gen_events = [e for e in all_events if year <= e["year"] < year + 40]
        for ev in gen_events:
            if ev.get("type") == "ahistorical":
                # Use ahistorical effect dispatcher (world-context-aware)
                shock_add, tech_accel_temp, religion_strength = apply_ahistorical_event_effect(
                    ev, shock_add, tech_accel, religion_strength, world_cfg, rng)
                # Apply permanent tech_factor change to world
                if ev["dim"] in ("tech_collapse", "tech_unknown"):
                    tech_accel = tech_accel_temp
                # Per-agent secondary effects
                sec = get_event_secondary_effects(ev)
                if "edu_multiplier" in sec:
                    secondary_effects_this_gen["edu_multiplier_global"] *= sec["edu_multiplier"]
                if "asset_random_loss" in sec:
                    secondary_effects_this_gen["asset_random_loss_global"] += sec["asset_random_loss"]
                if "inst_random_delta" in sec:
                    secondary_effects_this_gen["inst_random_delta_global"] += sec["inst_random_delta"]
                if "direct_mortality_p" in sec:
                    secondary_effects_this_gen["direct_mortality_p"] = max(
                        secondary_effects_this_gen["direct_mortality_p"],
                        sec["direct_mortality_p"])
                if "asset_lottery" in sec:
                    secondary_effects_this_gen["asset_random_loss_global"] += sec["asset_lottery"] * 0.5
            else:
                # Historical event: original handling
        ...
        # Encounter mechanism (4-channel) if active
        if encounter_active and encounter_year is not None and year >= encounter_year:
            generations_since = (year - encounter_year) // 40
            encounter_intensity = evolve_intensity(rng, encounter_intensity,
                                                     generations_since)
            shock_add, tech_accel, religion_strength, enc_secondary = apply_encounter_effects(
                encounter_intensity, 0.15,  # base magnitude
                shock_add, tech_accel, religion_strength, rng)
            # Merge encounter secondary effects
            secondary_effects_this_gen["edu_boost_global"] += enc_secondary["edu_boost"]
            secondary_effects_this_gen["inst_boost_global"] += enc_secondary["inst_boost"]
            secondary_effects_this_gen["trade_boost_global"] += enc_secondary["trade_boost"]
            secondary_effects_this_gen["direct_mortality_p"] = max(
                secondary_effects_this_gen["direct_mortality_p"],
                enc_secondary["direct_mortality_p"])
            encounter_log.append({
                "year": year, "intensity": encounter_intensity,
                "generations_since": generations_since,
                "summary": get_encounter_summary(encounter_intensity, generations_since),
            })
        ...
        # Apply ahistorical/encounter direct mortality (additive on dfp)
        if secondary_effects_this_gen["direct_mortality_p"] > 0:
            dfp = np.clip(dfp + secondary_effects_this_gen["direct_mortality_p"],
                           .015, .98)
        ...
        if len(survivors):
            ...
            # Apply v5.1 secondary effects from ahistorical/encounter events
            if secondary_effects_this_gen["edu_multiplier_global"] != 1.0:
                # Knowledge collapse: multiply existing edu before update
                edu[survivors] = edu[survivors] * secondary_effects_this_gen["edu_multiplier_global"]
            if secondary_effects_this_gen["edu_boost_global"] > 0:
                eg = eg + secondary_effects_this_gen["edu_boost_global"]
            if secondary_effects_this_gen["inst_boost_global"] > 0:
                ig = ig + secondary_effects_this_gen["inst_boost_global"]
            if secondary_effects_this_gen["trade_boost_global"] > 0:
                tg = tg + secondary_effects_this_gen["trade_boost_global"]
            ...
            # Apply v5.1 random asset loss (physical_anomaly, resource_shock)
            if secondary_effects_this_gen["asset_random_loss_global"] > 0:
                rand_loss_mag = secondary_effects_this_gen["asset_random_loss_global"]
                assets[survivors] = np.clip(
                    assets[survivors] - rng.uniform(-rand_loss_mag, rand_loss_mag,
                                                     size=len(survivors)),
                    0, 1)
            # Apply v5.1 institutional delta (political_break)
            if secondary_effects_this_gen["inst_random_delta_global"] != 0:
                inst_delta = secondary_effects_this_gen["inst_random_delta_global"]
    ...
    return {
        ...
        "ahistorical_events_sampled": ahistorical_events_list,
...
def main():
    ap = argparse.ArgumentParser(
        description="World Simulation v5.1 — Stage 1.5 with ahistorical events, "
                    "frequency modes, and full encounter mechanism")
    ...
    ap.add_argument("--outdir", default="outputs_v51")
    ap.add_argument("--frequency", default="sporadic",
                    choices=["sporadic", "frequent", "sparse"],
                    help="Event frequency mode (sporadic=baseline, frequent=×3, sparse=×0.3)")
    ap.add_argument("--no_ahistorical", action="store_true",
                    help="Disable ahistorical event catalog")
    ap.add_argument("--encounter", action="store_true",
                    help="Force encounter mechanism (year window 1480-1520)")
    args = ap.parse_args()
    ...
    for w in worlds:
        ...
        # Auto-enable encounter for unknown_encounter world
        encounter_window = None
        if args.encounter or w == "unknown_encounter":
            encounter_window = (1480, 1520)
        print(f"\n{'='*70}\nRunning: {cfg['name']}  (n={args.n}, freq={args.frequency})\n{'='*70}")
        t0 = time.time()
        ...
        n_ahist = len(res["ahistorical_events_sampled"])
        n_encounter_steps = len(res["encounter_log"])
        print(f"  Religion strength: {rs_init:.2f} -> {rs_final:.2f}")
        print(f"  Endogenous religious conflicts: {n_endo_conflicts}")
        print(f"  Ahistorical events realized: {n_ahist}")
        if n_encounter_steps > 0:
            print(f"  Encounter mechanism activated: {n_encounter_steps} generations affected")
    ...
    with open(out / "all_world_results_v51.json", "w") as f:
    ...
    if len(worlds) > 1:
        ...
        df_cross.to_csv(out / "cross_world_comparison_v51.csv", index=False)
