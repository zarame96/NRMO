...
\subsection{Hold State / Data-Reliability Signal
(Historical Heading: Hold Mode)}
\label{sec:hstn-hold-c2}

\nrmobox{Canonical typing}{
The historical heading ``Hold Mode'' is retained here only as provenance.
HST-N \texttt{Hold} is a data-reliability state/signal and notification;
it is \textbf{not} an Operational Mode. Canonical governance/lifecycle
\texttt{HOLD} likewise denotes a state/result requiring resolution or
human intervention, not a peer mode.
}
...
HST-N's ``Hold'' is a notification to downstream governance, not an
executive action. HST-N does not acquire decision, governance, or veto
