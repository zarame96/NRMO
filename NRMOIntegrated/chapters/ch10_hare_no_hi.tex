...
\textbf{Status}: OPERATIONS --- Context Override\\[0.3em]
\textbf{Provenance}: Earlier revisions called HARE a ``Context Override
Mode.'' Under the Normative Canon, HARE is a Context Override, not an
additional ordinary Operational Mode.\\[0.3em]
...
\begin{itemize}
\item Hare-no-Hi is a Context Override under the Normative Canon. The
older description placing it as a ``Context Override Mode'' in the Type
ZERO mode hierarchy is preserved only as historical terminology. HARE
activates non-resident under explicit declaration and terminates by time
or trigger; it does not join the canonical Operational Mode set
\texttt{NORMAL / VENTURE / MISSION / SAFE}.
